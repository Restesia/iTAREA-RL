...
def solve(nTask, nNodes):
    ...
    percentageCPU = [[solver.addVar(vtype=GRB.INTEGER, name=f"pCPU_{r}_{c}", lb=0, ub=cpuPercentages * int(nodes[r]['cores']))
                      for c in range(nTask)] for r in range(nNodes)]

    # {node, % core assigned to task} This variable is used to avoid the division between variables (not supported by default)
    percentageCPUaux = [[solver.addVar(vtype=GRB.CONTINUOUS, name=f"auxPCPU_{r}_{c}", lb=0,
                                       ub=cpuPercentages * int(nodes[r]['cores'])) for c in range(nTask)]
                        for r in range(nNodes)]

    # Edges of the task-call graph
    constraints = [[0 for c in range(nTask)] for r in range(nConstraints)]

    # Defining the time constraints (to assure the QoS).
    ...
    for t in range(nTask):
        ...
        for n in range(nNodes):
            solver.addConstr(
                ((assignment[t][n]) * (tasks[t][4] <= nodes[n][8]) * (tasks[t][5] <= nodes[n][9]) * (
                            tasks[t][3] <= nodes[n][1]) * (tasks[t][8] == nodes[n][10]) * (
                                (tasks[t][7] == 'none') + (tasks[t][7] == nodes[n][11])) == assignment[t][n]))
    ...
    for m in range(nNodes):
        ...
        solver.addConstr(quicksum(percentageCPU[m]) <= cpuPercentages * int(nodes[m][14]))
        solver.addConstr(
            quicksum(assignment[t][m] * tasks[t][1] for t in range(nTask)) <= int(nodes[m][4]))  # Checking RAM
    ...
    for c in range(nConstraints):
        ...
        solver.addConstr(
            communicationTime == quicksum(
                quicksum(
                    quicksum(
                        assignment[constraints[c][i]][m] * (1 - assignment[constraints[c][j]][m]) * (
                                    ((relation[constraints[c][i]][constraints[c][j]] * 1000000) /
                                     int(nodes[m][1])) + (rtt[0][0] * 1000000)) for j in
                        range(2, 2 + constraints[c][1])) for i in range(2, 2 + constraints[c][1])) for m in
                range(nNodes)))
        solver.addConstr(
            computationTime == quicksum(
                quicksum(
                    (assignment[constraints[c][i]][m] * (
                                (tasks[constraints[c][i]][0] * 1000000) * (percentageCPUaux[m][
                            constraints[c][i]]) * int(nodes[m][14]) * cpuPercentages)) / int(nodes[m][0]) for i in
                    range(2, 2 + constraints[c][1])) for m in range(nNodes)))
    ...
    solver.addConstr(
        communicationCost == quicksum(
            quicksum(
                quicksum(
                    assignment[i][m] * (1 - assignment[j][m]) * (
                                nodes[m][2] * (relation[i][j] / int(nodes[m][1])) * nodes[m][5]) for j in
                    range(nTask)) for i in range(nTask)) for m in range(nNodes)))
    solver.addConstr(
        computationCost == quicksum(
            quicksum(
                assignment[i][m] * ((100 - int(nodes[m][15])) / 100) * nodes[m][3] * (
                            (tasks[i][0] * 10000) / int(nodes[m][0])) for i in range(nTask)) for m in
            range(nNodes)))
    solver.addConstr(
        communicationCostDown == quicksum(
            quicksum(
                quicksum(
                    assignment[i][m] * (1 - assignment[j][m]) * (
                                nodes[m][6] * (relation[j][i] / int(nodes[m][7])) * nodes[m][5]) for j in
                    range(nTask)) for i in range(nTask)) for m in range(nNodes)))
    ...
    if solver.status != 3:
        for v in solver.getVars():
            ...
            if str(v.varName[0:4]) == 'pCPU' and v.x > 0:
                aux = re.findall('\d+', v.varName)
                result += 'CPU assigned in Node ' + str(aux[0]) + ' to Task ' + str(aux[1]) + '. CPU (cores): ' + str(
                    int(v.x) / cpuPercentages) + '\n'
    else:
...
if __name__ == "__main__":
    ...
    nUsers_arg = sys.argv[1]  # Argumento 1 (nUsers)
    nConstraints_arg = sys.argv[2]  # Argumento 2 (nConstraints)
    nTask_arg = sys.argv[3]  # Argumento 3 (nTask)
    nNodes_arg = sys.argv[4]  # Argumento 4 (nNodes)
    cpuPercentages_arg = sys.argv[5]  # Argumento 5 (cpuPercentages)
    nodes_arg = sys.argv[6]
    tasks_arg = sys.argv[7]
    relation_str = sys.argv[8]
    rtt_str = sys.argv[9]
    
    nodes = json.loads(nodes_arg)
    tasks = json.loads(tasks_arg)
    relation = json.loads(relation_str)
    rtt = json.loads(rtt_str)
    nUsers = int(nUsers_arg)  # Convertir los argumentos de cadena a su tipo original
    tasks = int(nConstraints_arg)
    nTask = int(nTask_arg)
